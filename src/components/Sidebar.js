...
const navigation = [
  ...
  { name: 'Users', href: '/dashboard/users', icon: UsersIcon },  // ✅ ADD THIS
  { name: 'Products', href: '/products', icon: ShoppingBagIcon },
  { name: 'Categories', href: '/categories', icon: TagIcon },
  { name: 'Deals', href: '/dashboard/deals', icon: TicketIcon },  // ✅ ADD THIS
  { name: 'Sales', href: '/dashboard/sales', icon: TicketIcon },  // ✅ ADD THIS
  { name: 'Newsletter', href: '/newsletter', icon: EnvelopeIcon },  // ✅ ADD THIS
  { name: 'Support Tickets', href: '/dashboard/support', icon: ChatBubbleLeftRightIcon },
  ...
  return (
    <>
      ...
      <div className={`fixed inset-y-0 left-0 z-30 w-64 overflow-y-auto bg-gray-900 transition duration-300 transform lg:translate-x-0 lg:static lg:inset-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <nav className="mt-10">
          {navigation.map((item) => {
            // ✅ FIX: Check if pathname starts with item.href for nested routes
